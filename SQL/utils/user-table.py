...
cursor.execute ("CREATE TABLE `User` (`userID` VARCHAR(100) PRIMARY KEY,`email` VARCHAR(255) NOT NULL UNIQUE,`firstName` TEXT NOT NULL,`lastName` TEXT NOT NULL, `password` TEXT NOT NULL,`createdAt` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,`updatedAt` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, CONSTRAINT `unique_email` UNIQUE (`email`(255)));")
